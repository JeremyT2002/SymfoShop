...
final class Version20260226120000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        // Re-runnable after partial schema creation.
        ...
        if ($isSqlite) {
            ...
            // MySQL/MariaDB: avoid using PostgreSQL-specific SERIAL types.
            $idType = $isPostgres ? 'SERIAL' : 'INTEGER';
            $timestampType = $isPostgres ? 'TIMESTAMP(0)' : 'DATETIME';

            // `user` is a reserved keyword; escape per platform.
            $userTable = $isPostgres ? '"user"' : '`user`';
            ...
            $this->addSql('CREATE TABLE theme_revision (
                id '.$idType.' PRIMARY KEY NOT NULL,
                theme_id '.$idType.' NOT NULL,
                config JSON NOT NULL,
                version INT NOT NULL,
                status VARCHAR(20) DEFAULT \'draft\' NOT NULL,
                published_at '.$timestampType.' DEFAULT NULL,
                comment VARCHAR(500) DEFAULT NULL,
                created_by_id '.$idType.' DEFAULT NULL,
                created_at '.$timestampType.' NOT NULL,
                CONSTRAINT FK_theme_revision_theme FOREIGN KEY (theme_id) REFERENCES theme (id) ON DELETE CASCADE,
                CONSTRAINT FK_theme_revision_created_by FOREIGN KEY (created_by_id) REFERENCES '.$userTable.' (id) ON DELETE SET NULL
            )');
            $this->addSql('CREATE INDEX idx_theme_revision_theme ON theme_revision (theme_id)');
            $this->addSql('CREATE INDEX idx_theme_revision_theme_version ON theme_revision (theme_id, version)');
        }
