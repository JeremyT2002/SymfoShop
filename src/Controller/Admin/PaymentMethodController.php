...
class PaymentMethodController extends AbstractController
{
    ...
    {
        if (!$this->isCsrfTokenValid('bulk_payment_methods_active', (string) $request->request->get('_token'))) {
            $this->addFlash('error', 'Invalid CSRF token.');
        ...
        $methods = $this->paymentMethodRepository->findBy(['id' => $ids]);
        foreach ($methods as $method) {
            $method->setIsActive($active);
        }
        $this->entityManager->flush();
        $this->addFlash('success', sprintf('Updated %d payment method(s).', count($methods)));
